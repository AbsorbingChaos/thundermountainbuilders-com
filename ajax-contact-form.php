...
class Ajax_Contact_Form {
     ...
     * @var string
     */
    protected $address_destination = 'contact@thundermountainbuilders.com';
    /**
     * Message subject
     *
     * @var string
     */
    protected $message_subject = 'New Web Lead';
    /**
    ...
    protected $strings = array(
        'body'              => '
            <p>New Web Lead from contact form on ThunderMountainBuilders.com, info below:</p>
            <p><strong>Name:</strong> {{name}}</p>
            <p><strong>Job Type:</strong> {{jobtype}}</p>
            <p><strong>Email:</strong> {{email}}</p>
            <p><strong>Phone:</strong> {{phone}}</p>
            <p><strong>Additional Info:</strong> <br> {{message}}</p>',
        'success'           => 'Thank you for submitting your info! We will be in touch! Feel free to view our portfolio below.',
        ...
        'enter_jobtype'     => 'Please select a Job Type.',
    ...
    public function __construct() {
        ...
        // Ajax check. REMOVED - JS - 20190310
        ...
        $jobtype = stripslashes(trim($_POST['jobtype']));
        $message = stripslashes(trim($_POST['addinfo']));
        // Sanitize fields.
        ...
        if ( ! $phone ) {
            $this->errorHandler('enter_phone');
        }
        // Check if job type has been selected.
        if ( $jobtype == "none" ) {
            $this->errorHandler('enter_jobtype');
        }
        ...
        $result = @mail($this->address_destination, $this->message_subject . ": " . $name . " - " . $jobtype, $body, $headers);
        if ( $result ) {
